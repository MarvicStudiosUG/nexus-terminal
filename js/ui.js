...
export async function randomFact() {
  ...
  const cached = cacheGet(key);
  if (cached) {
    add('<div class="block">' + esc(cached) + '</div>', 'ok');
    return;
  }
...
// --- Wikipedia ---
export async function wikiSearch(term) {
  const key = 'wiki_' + term;
  const cached = cacheGet(key);
  if (cached) {
    add('<div class="block"><strong>' + esc(cached.title) + '</strong><br>' + esc(cached.extract.substring(0, 400)) +
      '...</div>', 'ok');
    return;
  }
  ...
  try {
    ...
    if (data.type === 'disambiguation') {
      unloader(lid);
      add('Disambiguation page.', 'warning');
      return;
    }
    const result = { title: data.title, extract: data.extract || '' };
    cacheSet(key, result, 3600000);
    unloader(lid);
    add('<div class="block"><strong>' + esc(data.title) + '</strong><br>' + esc((data.extract || '').substring(0,
      400)) + '...</div>', 'ok');
  } catch {
    unloader(lid);
...
export async function defineWord(word) {
  ...
  if (cached) {
    ...
    cached.meanings.forEach(meaning => {
...
export async function randomUser() {
  ...
  if (cached) {
    ...
    add('<div class="block"><strong>' + esc(u.name.first) + ' ' + esc(u.name.last) +
      '</strong><br>Email: ' + esc(u.email) + '<br>Location: ' + esc(u.location.city) + ', ' + esc(u.location
        .country) + '</div>', 'ok');
    return;
  }
  ...
  try {
    ...
    const data = await res.json();
    const u2 = data.results[0];
    cacheSet(key, u2, 600000);
    unloader(lid);
    add('<div class="block"><strong>' + esc(u2.name.first) + ' ' + esc(u2.name.last) +
      '</strong><br>Email: ' + esc(u2.email) + '<br>Location: ' + esc(u2.location.city) + ', ' + esc(u2.location
        .country) + '</div>', 'ok');
  } catch {
    unloader(lid);
...
// --- Bored Activity ---
export async function boredActivity() {
  const key = 'bored';
  const cached = cacheGet(key);
  if (cached) {
    add('<div class="block"><strong>Activity:</strong> ' + esc(cached.activity) + '<br>Type: ' + esc(cached.type) +
      ' | Participants: ' + cached.participants + '</div>', 'ok');
    return;
  }
  ...
  try {
    ...
    cacheSet(key, data, 600000);
    unloader(lid);
    add('<div class="block"><strong>Activity:</strong> ' + esc(data.activity) + '<br>Type: ' + esc(data.type) +
      ' | Participants: ' + data.participants + '</div>', 'ok');
  } catch (e) {
    unloader(lid);
